...
class MockNatsClient extends EventEmitter {
	 ...
	 *
	 * TODO: Add support for queue groups
	 * 
	...
	_findSubscribesBySubject(subject) {		
		...
		// Pick one subscribe from each queue group
		// Should always publish to subscription if no queue group is set
		let onePerQueue = {};
	...
	_findSubscribeByQueue(queue) {
		return this.subs.find(sub => sub.queue === queue);
	}
}
